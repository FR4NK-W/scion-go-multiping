...
def plot_rtt_ratio():
    # Load the CSV files
    scion_df = pd.read_csv("avg_scion_pings_pairs.csv", parse_dates=["hour"])
    ip_df = pd.read_csv("avg_ip_pings_pairs.csv", parse_dates=["hour"])
    
    # Merge datasets on timestamp
    merged_df = pd.merge(scion_df, ip_df, on="hour", suffixes=("_scion", "_ip"))
    ...
    sns.lineplot(x=merged_df["hour"], y=merged_df["smoothed_rtt_ratio"], label="SCION/IP RTT Ratio", linestyle="-")
    ...
    unique_days = merged_df["hour"].dt.normalize().unique()
    every_Second_day = unique_days[::2]
    plt.xticks(every_Second_day, rotation=45)
