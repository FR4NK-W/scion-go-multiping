...
def plot_averaged_simulation_results(percentages_links_removed, average_as_with_path_1, average_as_with_path_2):
    ...
    plt.xlabel('Fraction of Links Removed (%)',  fontsize=14)
    plt.ylabel('AS Pairs with Connectivity (%)',  fontsize=14)
...
if __name__ == "__main__":
    ...
    for i in range(1, 21):
        simulation_files_multipath.append(f"data_percentage/simulation_results_{i}.json")
    
    # List of simulation result files for the second set (Single-path)
    simulation_files_singlepath = []
    for i in range(1, 21):
        simulation_files_singlepath.append(f"data_percentage/simulation_results_{i}_single.json")
